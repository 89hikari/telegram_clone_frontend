...
const MainWindow: React.FC = () => {
    ...
    const { sidebar, currentMessages, peerInfo } = useAppSelector((state: RootState) => state.messages);
    ...
    useEffect(() => {
        if (user.id) {
            const socketCallback = (msg: any) => dispatch(catchNonExistendMessageFromSocket(msg));
            socket.off(`new_messages_${user.id}`, socketCallback).on(`new_messages_${user.id}`, socketCallback); // workaround to get rid of doubles
            return () => {
