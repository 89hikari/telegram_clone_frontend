...
export const getSidebarLastMessages = createAsyncThunk<any, any, { state: RootState }>(
    "messages/getSidebarLastMessages",
    async (_, { getState }) => {
        const rootState = getState();
...
export const sendMessage = createAsyncThunk<any, any, { state: RootState }>(
    ...
    async ({ message, receiverId }: { message: string, receiverId: number }, { getState }) => {
        const rootState = getState();
        const response = api.post(createBaseUrl('messages'), {
            message: message,
